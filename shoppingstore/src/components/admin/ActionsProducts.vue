<template>
  <div>
    <h4 class="text-center text-white p-2"
      v-bind:class="themeClass">{{ editMode ? "Edit Product" : "Create Product" }}</h4>
    ...
    <div class="text-center">
      <router-link to="/admin/products" class="btn btn-secondary m-1"><i class="fa fa-times-circle"></i><b class="text-title-icon">Cancel</b></router-link>
      <button class="btn m-1" v-bind:class="themeClassButton" v-on:click="handleSave"><i v-bind:class="themeClassIcon"></i><b class="text-title-icon">{{ editMode ? "Save Changes" : "Store Product"}}</b></button>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
      ...
      computed: {
        ...
        themeClassIcon() {
            return this.editMode ? "fa fa-check-circle" : "fa fa-download";
        }
      },
      validations: {
        product: {
